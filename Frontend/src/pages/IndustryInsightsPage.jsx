...
export default function IndustryInsightsPage() {
  const [insights, setInsights] = useState({
    ...
    industryGrowth: "", // Initially empty
    demandLevel: "", // Initially empty
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/chat", { message: userQuery });
      const mentorResponse = res.data.response;

      // Update insights based on mentor response
      const updatedInsights = { ...insights };
      if (mentorResponse.toLowerCase().includes("growth")) {
        updatedInsights.industryGrowth = `${Math.random() * 10}%`; // Dynamic update
      } else if (mentorResponse.toLowerCase().includes("demand")) {
        updatedInsights.demandLevel = Math.random() > 0.5 ? "High" : "Medium";
      ...
      setChatMessages((prev) => [...prev, { text: mentorResponse, sender: "mentor" }]);
    } catch (err) {
  ...
  return (
    <div
      className="min-h-screen p-10 pt-16"
      style={{
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gray-800 bg-opacity-80 p-4 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-300">Next update {insights.nextUpdate}</p>
        </div>
        ...
        <div className="bg-gray-800 bg-opacity-80 p-4 rounded-lg shadow-lg">
          ...
          {insights.industryGrowth ? (
            ...
            <p className="text-gray-400 italic">Ask about industry growth to see data</p>
          )}
        </div>
          ...
          {insights.demandLevel ? (
            <>
              <div className="w-full bg-gray-600 rounded-full h-2.5">
                <div
                  className="bg-green-500 h-2.5 rounded-full"
                  style={{ width: insights.demandLevel === "High" ? "75%" : "50%" }}
                ></div>
              </div>
              <p>{insights.demandLevel}</p>
            </>
          ) : (
            <p className="text-gray-400 italic">Ask about demand level to see data</p>
          )}
        </div>
          ...
          {chatMessages.map((msg, index) => (
            <div
              ...
              className={`my-2 p-2 rounded ${
                msg.sender === "user" ? "bg-purple-600" : "bg-gray-700 bg-opacity-80"
              }`}
            >
              {msg.text}
            </div>
      ...
      <p className="text-sm text-gray-300 mt-4">Last updated: {insights.lastUpdated}</p>
    </div>
  );
}